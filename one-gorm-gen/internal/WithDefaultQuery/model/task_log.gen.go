...
type TaskLog struct {
	...
	TaskLogName  string    `gorm:"column:task_log_name;not null" json:"taskLogName"`
	Input        *string   `gorm:"column:input;comment:输入：wiki地址" json:"input"`                                          // 输入：wiki地址
	IsNamed      bool      `gorm:"column:is_named;not null;comment:是否被手动命名过，没有的话就要存入wiki里面的标题" json:"isNamed"`           // 是否被手动命名过，没有的话就要存入wiki里面的标题
	...
	StepNow      int32     `gorm:"column:step_now;not null;comment:这个任务执行到了哪一步" json:"stepNow"` // 这个任务执行到了哪一步
}
